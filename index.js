	...
	if (
		...
		fn.toString = (function toString() {
			...
			if (
				...
				(!/^(get|set|async|function)/.test(functionString) || !functionString.slice(/([^]*?)[\s/(]/.exec(functionString)[1].length).replace(/\s/g, '').replace(/(\/\*[^]*?\*\/|\/\/[^]*?\n)\s*/g, '').trim().startsWith('('))
			) {
			...
			if (/^(get|set|async|function)/.test(functionString)) {
				return name + functionString.slice(/([^]*?)[\s/(]/.exec(functionString)[1].length);
			}
		}).bind(fn);
	}
